...
- CPC = 广告花费 / 点击量
- 广告转化率 = 广告订单量 / 点击量
- 近七天数据（最新主图） = 主区间末端最近 7 天 CTR

“近七天数据（最新主图）”与 CTR 一样，按小数写入飞书数字/百分比字段。
例如 3.25% 写入 0.0325。

当前 DWS 尚无 Sessions 字段，因此不计算、不校验、不更新“转化率”。
"""
...
def resolve_source_layout(schema: str, table: str) -> str:
...
def query_metrics(
    ...
    sql = f"""
        SELECT
            ...
            SUM(
                CASE
                    WHEN {dt_sql} BETWEEN %s AND %s
                    THEN COALESCE({clicks_sql}, 0)
                    ELSE 0
                END
            ) AS recent_clicks,
            SUM(
...
async def run(args: argparse.Namespace) -> None:
    ...
    product_expr = resolve_source_layout(schema, table)
    ...
    if wrong_numeric_types:
        raise RuntimeError(
            "以下飞书回写字段必须是数字类型: "
            + ", ".join(sorted(wrong_numeric_types))
        )
    ...
    for record in records:
        ...
        if end_date < start_date:
            logger.warning(
                f"跳过 {spu}: 区间结束日 {end_date} 早于更换日期 {start_date}"
            )
            skipped += 1
        ...
        logger.info(
            ...
            f"近7天={recent_start}~{recent_end} | "
            f"rows={metrics['source_rows']} | "
            f"点击量={metrics['点击量']} | "
            f"CTR={metrics['CTR']} | "
            f"CPC={metrics['CPC']} | "
            f"广告转化率={metrics['广告转化率']} | "
            f"近7天CTR={metrics['近七天数据（最新主图）']}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-table",
        default="dws_op_listing_traffic_daily",
    )
    parser.add_argument(
        "--country",
        default="US",
